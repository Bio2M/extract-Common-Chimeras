...
## goal: extract common chimeras detected after the chimeraPipeline process of crac-tools (chimCT)
...
=head1 NAME
    ...
    extractCommonChimeras.pl - Extract common chimeras from a list of csv files generated by chimCT (crac-tools). By default, we only consider redundant chimeras, in at least 2 different samples, but the threshold can be modified.
...
=item 11 B<profil_support>     -

=item 12 B<profil_location>    -
...
=head1 AUTHOR
    ...
    Nicolas PHILIPPE, Chloe BESSIERE <chloe.bessiere@inserm.fr>
...
#    --min-cover: (undef by default)


my ($help, $version, $min_score, $min_cover, $max_cover, $summary_file, $min_redundant, $max_redundant, $output_file, $r_file, $filter_file) = (0, 0, 50, 0, undef, undef, 2, undef, undef, undef, undef);

GetOptions( 	"min-ranking=i" => \$min_score, # score between 0 and 100
		"min-cover=i"   => \$min_cover, #not defined ? raw spanning junction reads ?
